...
object Scoreboard {
    ...
    fun renderScoreboard(objective: ScoreObjective) {
        if (VanillaHUDConfig.scoreboard) {
            ...
            withTranslate(
                (-scorePointRight).toFloat() * (VanillaHUDConfig.scoreboardScale - 1f),
                (-(bottom - collection.size * UMinecraft.getFontRenderer().FONT_HEIGHT)).toFloat() * (VanillaHUDConfig.scoreboardScale - 1f),
                0.0F
            ) {
                withScale(VanillaHUDConfig.scoreboardScale, VanillaHUDConfig.scoreboardScale, 1.0F) {
