...
public abstract class MountHealth_GuiIngame_Mixin {
    ...
    private int mountAlignment(int x) {
        ...
        return VanillaHUD2.getMount().getAlignment() == 1 ? 81 + x : -(x + 9);
    }
    ...
    private void mountFlip(GuiIngameForge instance, int x, int y, int textureX, int textureY, int width, int height) {
        ...
        if (textureX < 97 || ((textureX - 16) / 9) % 2 == 0 || VanillaHUD2.getMount().getAlignment() == 1) {
            instance.drawTexturedModalRect(x, y, textureX, textureY, width, height);
        } else {
    ...
    private int mountMode(int constant) {
        ...
        return (VanillaHUD2.getMount().getMode() == 1 ? 1 : -1) * 10;
    }
    ...
    private void mount(CallbackInfo ci) {
        ...
        MountHealthHUD hud = VanillaHUD2.getMount();
        if (hud.getHidden()) {
            ci.cancel();
            post(HEALTHMOUNT);
            return;
        ...
        GlStateManager.translate((int) hud.getX() + 182, (int) hud.getY() - (hud.getHealthLink() ? VanillaHUD2.getHealthLinkAmount() : 0) - right_height, 0F);
